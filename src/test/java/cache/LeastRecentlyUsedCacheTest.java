...
public class LeastRecentlyUsedCacheTest {
    ...
    public void lruCacheGetSetTest() {
        ...
        Assertions.assertThat(itemByCache).isEqualTo(item);
    }
    ...
    public void lruCacheOrderChangeWhenAddedAndUpdatedTest() {}

    @Test
    @DisplayName("LRU 캐시 - 데이터를 읽었을 때 최근 사용한 것으로 순서가 변경되어야 한다")
    public void lruCacheOrderChangeWhenReadTest() {}

    @Test
    @DisplayName("LRU 캐시 - 데이터를 조회했으나 캐시에 존재하지 않을 경우 DB에서 데이터를 조회 후 캐시에 데이터를 추가해야 한다")
    public void lruCacheReadFailTest() {
    }
